...
import Erroe404 from '../../Error404.page'
...
const InitialState = ({postId, value, setValue, handleConfirm, setModalOpen}) => {
  ...
  const { data: GPrice, isLoading: LPrice, isSuccess: SPrice, isError: EPrice } = useContractRead({
    ...PriceConversion,
  ...
  useEffect(() => {
    if (GPrice) {
      const price = Number(GPrice[0])/(10**GPrice[1])
      setLatestPrice(price.toFixed(2))
    }
  }, [GPrice])
