...
export const CustomConnect = ({ isHeader }: {isHeader: boolean}) => {
  const navigate = useNavigate();

  const redirectOrConnect = (isConnected: boolean, openConnectModal: ()=>void) => {
    if (isConnected) {
          ...
          >
            {!isHeader ? (
              <Btn onClick={() => redirectOrConnect(connected, openConnectModal)} type="button">
                {!connected ? "Join Us" : "Open Dashboard"}
              </Btn>
            ) : (
              <>
                {!connected ? (
                    ...
                    {chain.unsupported ? (
                      ...
                      <BtnSmall onClick={() => openAccountModal()} type="button" className={"rounded-l-none"}>
                        <div className={"w-6 h-6 border-2 border-gray-300 rounded-full bg-gray-200 p-0.5 mr-2 opacity-80"}>
                          {account.ensAvatar ? (
                            <img src={account.ensAvatar ? account.ensAvatar : ""} alt="" className={"w-6 h-6"}/>
                          ) : (
